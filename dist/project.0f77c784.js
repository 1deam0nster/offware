...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (navItemWithSubmenu && navLinkWithSubmenu) {
        ...
        document.addEventListener('click', (e)=>{
            ...
            if (!navItemWithSubmenu.contains(e.target)) navItemWithSubmenu.classList.remove('active');
        });
    }
    ...
    if (productCards.length > 0 && productExpandButtons.length > 0) productCards.forEach((card, index)=>{
        // Инициализация: все карточки свернуты
        if (!card.classList.contains('collapsed') && !card.classList.contains('expanded')) card.classList.add('collapsed');
    ...
    if (glowBorderCards.length > 0) {
        ...
        document.addEventListener('mousemove', updateGlowBorderCardsMouse, {
            passive: true
        });
    }
        ...
        serviceTabs.forEach((tab)=>{
            ...
            if (tabsContainer && !sections.has(tabsContainer)) {
                ...
                if (activeTab) {
                    ...
                    if (firstContentBox) firstContentBox.style.display = 'grid';
                }
            }
        });
    }
...
},{}]},["7wZbQ","2R06K"], "2R06K", "parcelRequire716c", {})
